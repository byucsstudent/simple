# Web Application Development

**Discipline:** Software engineering
**Problem type:** Enterprise website creation
**Difficulty:** 3 / 5
**Modes:** Practice
**Instability:** Off

## Engagement Description

This assessment allows you to demonstrate your ability to apply software engineering principles and architecture to a real life scenario of building a web application.


## Learning Outcomes

- **Full-Stack Web Application Development**: Design and implement interactive, full-stack web applications using modern web technologies, applying computational thinking and user-centered design to solve meaningful problems that serve others.
- **Web Architecture**: Design and reason about web architectures, including client-server communication, APIs, authentication, real-time interaction, and data persistence, with an awareness of the responsibility to build systems that meet present needs and continue to serve well as those needs change over time.
- **Security and Societal Impact**: Apply secure coding practices and evaluate the ethical implications of web technologies, including privacy, accessibility, and eternal impact of the systems they build.
- **Deployment and Application Management**: Deploy and manage web applications, demonstrating responsibility for reliability, performance, and the experience of real users in production environments.

## Assessment Definition

```json
{
  "title": "Web Application Development",
  "discipline": "Software engineering",
  "problemType": "Enterprise website creation",
  "difficulty": 3,
  "practiceMode": true,
  "finalMode": false,
  "instability": false,
  "engagementDescription": "This assessment allows you to demonstrate your ability to apply software engineering principles and architecture to a real life scenario of building a web application.\n",
  "learningOutcomes": "- **Full-Stack Web Application Development**: Design and implement interactive, full-stack web applications using modern web technologies, applying computational thinking and user-centered design to solve meaningful problems that serve others.\n- **Web Architecture**: Design and reason about web architectures, including client-server communication, APIs, authentication, real-time interaction, and data persistence, with an awareness of the responsibility to build systems that meet present needs and continue to serve well as those needs change over time.\n- **Security and Societal Impact**: Apply secure coding practices and evaluate the ethical implications of web technologies, including privacy, accessibility, and eternal impact of the systems they build.\n- **Deployment and Application Management**: Deploy and manage web applications, demonstrating responsibility for reliability, performance, and the experience of real users in production environments.",
  "scenarioTitle": "Enterprise website creation",
  "scenarioGoal": "Create a SPA n-tier web app using React and Supabase.\n\nThe outcomes should include a demonstration of:\n\nFull-Stack Web Application Development: Design and implement interactive, full-stack web applications using modern web technologies, applying computational thinking and user-centered design to solve meaningful problems that serve others.\n\nWeb Architecture: Design and reason about web architectures, including client-server communication, APIs, authentication, real-time interaction, and data persistence, with an awareness of the responsibility to build systems that meet present needs and continue to serve well as those needs change over time.\n\nSecurity and Societal Impact: Apply secure coding practices and evaluate the ethical implications of web technologies, including privacy, accessibility, and eternal impact of the systems they build.\n\nDeployment and Application Management: Deploy and manage web applications, demonstrating responsibility for reliability, performance, and the experience of real users in production environments."
}
```
